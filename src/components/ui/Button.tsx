...
type Props = { children: React.ReactNode; to?: string; variant?: 'primary' | 'secondary' | 'text'; className?: string; onClick?: () => void };
export function Button({ children, to, variant = 'primary', className = '', onClick }: Props) {
  const styles = {
  ...
  return to ? <Link className={shared} to={to} onClick={onClick}>{content}</Link> : <button className={shared} onClick={onClick}>{content}</button>;
}
